...
const COLORS = ['var(--color-primary-500)', 'var(--color-secondary-500)', 'var(--color-success-500)', 'var(--color-warning-500)', 'var(--color-danger-500)', 'var(--color-primary-700)'];
    ...
    <Layout title="Platform Reports & Analytics">
      <div className="space-y-6 fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="stat-card bg-card border-l-4 border-primary-500">
          ...
          <div className="stat-card bg-card border-l-4 border-success-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground mb-1">Active Clients</p>
                <p className="text-3xl font-bold text-foreground">{tenants.filter(t => t.status === 'ACTIVE').length}</p>
              </div>
          ...
          <div className="stat-card bg-card border-l-4 border-warning-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground mb-1">Total Customers</p>
                <p className="text-3xl font-bold text-foreground">{tenants.reduce((sum, t) => sum + t.customer_count, 0)}</p>
              </div>
          ...
          <div className="stat-card bg-card border-l-4 border-primary-500">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-foreground">₹{revenueData.reduce((sum, d) => sum + d.revenue, 0)}</p>
              </div>
              <div className="w-12 h-12 rounded-full gradient-primary flex items-center justify-center">
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={revenueData}>
                  ...
                  />
                  <Legend />
                  <Bar dataKey="revenue" fill="var(--color-primary-500)" />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
        ...
        <Card className="bg-card border border-border">
          <CardHeader>
            <CardTitle className="text-foreground">Recent Subscription Activities</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {recentLogs.map((log) => (
                <div key={log.id} className="p-4 bg-muted/50 rounded-lg flex items-center justify-between border border-border">
                  <div>
                    <p className="font-medium text-foreground">Tenant: {log.tenant_id.slice(0, 8)}</p>
                  ...
                  <div className="text-right">
                    <p className="font-bold text-success-600">₹{log.amount}</p>
                    <p className="text-xs text-muted-foreground">{log.payment_status}</p>
                  </div>
                </div>
              ))}
              {recentLogs.length === 0 && (
                <p className="text-center text-muted-foreground py-4">No subscription activities yet</p>
              )}
            </div>
          </CardContent>
        </Card>
